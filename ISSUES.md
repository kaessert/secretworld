...
#### Problem (Resolved)
The `--demo` flag now properly skips character creation when combined with `--non-interactive` or `--json` flags.

#### Solution
Added `demo: bool = False` parameter to both `run_json_mode()` and `run_non_interactive()` functions. When `demo=True`, these functions use `create_demo_game_state()` instead of character creation/world generation.

**Changes**:
- `src/cli_rpg/main.py`:
  - Added `demo` parameter to `run_json_mode()` signature
  - Added `demo` parameter to `run_non_interactive()` signature
  - Both functions now use `create_demo_game_state()` when `demo=True`
  - Updated `main()` to pass `demo=parsed_args.demo` to both functions

**Tests Added**:
- `tests/test_demo_mode.py`: 6 new tests in `TestDemoModeWithNonInteractiveFlags` class

#### Usage
```bash
# All of these now work correctly
echo "look\nquit" | cli-rpg --demo --non-interactive
echo "quit" | cli-rpg --demo --json
```

#### Related Files
- `src/cli_rpg/main.py` - `run_json_mode()`, `run_non_interactive()`, `main()`
- `src/cli_rpg/test_world.py` - `create_demo_game_state()` function
- `tests/test_demo_mode.py` - Tests for demo mode with non-interactive flags
...
#### Problem (Resolved)
Need to verify that once terrain is generated by WFC for a coordinate, it is NEVER overwritten. Terrain should be immutable after generation.

#### Solution
Added terrain immutability safeguards to `ChunkManager` in `wfc_chunks.py`:

1. **`_synced` Flag**: Added `_synced: bool = False` attribute that tracks whether initial world setup is complete (set to `True` at end of `sync_with_locations()`)

2. **Defensive Warning**: Modified `set_tile_at()` to log a warning if called after `_synced` is True: "Terrain modification at (x, y) after sync - this may violate terrain immutability contract"

3. **`assert_terrain_unchanged()` Helper**: New method to validate terrain at a coordinate matches expected value; raises `AssertionError` with detailed message if terrain doesn't match

4. **Serialization**: `_synced` flag is persisted via `to_dict()`/`from_dict()` to maintain immutability state across save/load

**Design Decisions**:
- Warning instead of error to avoid breaking legitimate future terrain modification features
- Default `_synced=False` on deserialization ensures backward compatibility with older saves

**Files Modified**:
- `src/cli_rpg/wfc_chunks.py` - Added `_synced` flag, warning, helper method, serialization

**Tests Created**:
- `tests/test_terrain_immutability.py` - 14 new tests covering all terrain immutability scenarios

#### Acceptance Criteria
- [x] Confirm terrain is never overwritten (or find and fix the bug)
- [x] Add assertion in ChunkManager to prevent terrain overwrites
- [x] Add test that verifies terrain immutability
- [x] Location.terrain always matches chunk terrain at same coordinates
...
- `tests/test_terrain_immutability.py` - 14 terrain immutability tests
...
**Priority**: CRITICAL BLOCKER
**Date Added**: 2025-12-28
**Completed**: 2025-12-28 (commit 7ce5b01)

#### Problem (Resolved)
The exits shown by `look` command now dynamically calculate available directions based on passable terrain using `get_available_directions()` method instead of a static exits list.

#### Solution
The `Location.get_available_directions()` method now checks actual terrain passability from the ChunkManager instead of relying on a statically-stored exits list. This ensures exits are always consistent with the map display.

#### Related Files
- `src/cli_rpg/models/location.py` - `get_available_directions()` method
- `src/cli_rpg/game_state.py` - Uses dynamic exit calculation
...
- ✓ Quest prerequisites based on arc stage (see `npc_arc_quests.py`) - Quests can require minimum NPC relationship before becoming available
