...
class TestSubGridBoundsConfig:
    ...
    def test_bounds_config_has_town(self):
        """Town should have large 11x11 bounds."""
        assert SUBGRID_BOUNDS["town"] == (-5, 5, -5, 5)

    def test_bounds_config_has_dungeon(self):
        """Dungeon should have medium 7x7 bounds."""
        assert SUBGRID_BOUNDS["dungeon"] == (-3, 3, -3, 3)

    def test_bounds_config_has_cave(self):
        """Cave should have tiny 3x3 bounds."""
        assert SUBGRID_BOUNDS["cave"] == (-1, 1, -1, 1)

    def test_bounds_config_has_city(self):
        """City should have huge 17x17 bounds."""
        assert SUBGRID_BOUNDS["city"] == (-8, 8, -8, 8)

    def test_bounds_config_has_house(self):
        """House should have tiny 3x3 bounds."""
        assert SUBGRID_BOUNDS["house"] == (-1, 1, -1, 1)
...
class TestGetSubgridBounds:
    ...
    def test_returns_default_for_none(self):
        """None category should return default bounds."""
        assert get_subgrid_bounds(None) == (-2, 2, -2, 2)

    def test_returns_town_bounds(self):
        """Town category should return large bounds."""
        assert get_subgrid_bounds("town") == (-5, 5, -5, 5)

    def test_returns_dungeon_bounds(self):
        """Dungeon category should return medium bounds."""
        assert get_subgrid_bounds("dungeon") == (-3, 3, -3, 3)

    def test_returns_city_bounds(self):
        """City category should return huge bounds."""
        assert get_subgrid_bounds("city") == (-8, 8, -8, 8)
    ...
    def test_returns_default_for_unknown(self):
        """Unknown category should return default bounds."""
        assert get_subgrid_bounds("unknown_category") == (-2, 2, -2, 2)
        assert get_subgrid_bounds("xyz123") == (-2, 2, -2, 2)

    def test_all_configured_categories_return_valid_bounds(self):
        """All configured categories should return valid 4-tuple bounds."""
        for category in SUBGRID_BOUNDS:
            bounds = get_subgrid_bounds(category)
            assert len(bounds) == 4
            min_x, max_x, min_y, max_y = bounds
            assert min_x < max_x, f"{category}: min_x should be less than max_x"
            assert min_y < max_y, f"{category}: min_y should be less than max_y"
