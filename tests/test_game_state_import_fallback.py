"""Tests for game_state module AI import fallback behavior (lines 19-23).
...
class TestGameStateAIImportFallback:
    """Tests for AI import fallback behavior (lines 19-23)."""

    def test_ai_import_failure_sets_fallback_values(self):
        """Test that ImportError sets AI_AVAILABLE=False and mocks to fallback values.

        Spec: When ai_service/ai_world imports fail in game_state.py:
...
try:
    # Now import game_state - this should hit the except block
    import cli_rpg.game_state as test_game_state

    # Verify the except block set the fallback values
    assert test_game_state.AI_AVAILABLE is False, f"Expected AI_AVAILABLE=False, got {test_game_state.AI_AVAILABLE}"
    assert test_game_state.AIService is None, f"Expected AIService=None, got {test_game_state.AIService}"
    assert test_game_state.AIServiceError is Exception, f"Expected AIServiceError=Exception, got {test_game_state.AIServiceError}"
    assert test_game_state.expand_area is None, f"Expected expand_area=None, got {test_game_state.expand_area}"
...
'''
        ...
        try:
            ...
            env['COVERAGE_FILE'] = os.path.join(project_root, '.coverage.game_state_import_fallback')

            result = subprocess.run(
                [
                    sys.executable, '-m', 'coverage', 'run',
                    '--source=cli_rpg.game_state',
                    '--parallel-mode',
                    temp_file
                ],
                capture_output=True,
