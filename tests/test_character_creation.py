...
class TestGetManualStats:
    ...
    @patch('builtins.input', side_effect=["10", "12", "8", "14", "11", "10"])
    ...
    @patch('builtins.input', side_effect=["0", "10", "12", "8", "10", "10", "10"])
    ...
    @patch('builtins.input', side_effect=["21", "10", "12", "8", "10", "10", "10"])
    ...
    @patch('builtins.input', side_effect=["abc", "10", "12", "8", "10", "10", "10"])
...
class TestGenerateRandomStats:
    ...
    def test_random_stats_in_range(self):
        ...
        for _ in range(20):  # Test multiple times for randomness
            ...
            assert 8 <= stats["perception"] <= 15
            assert 8 <= stats["luck"] <= 15
...
class TestCreateCharacter:
    ...
    @patch('builtins.input', side_effect=["Hero", "1", "1", "10", "12", "8", "10", "10", "10", "yes"])
    def test_create_character_manual_valid_input(self, mock_input):
        ...
        # Flow: name -> class -> method -> stats (str, dex, int, cha, per, luck) -> confirm
        character = create_character()
        assert character is not None
        assert character.name == "Hero"
        assert character.character_class == CharacterClass.WARRIOR
        # Stats include Warrior bonuses: STR +3, DEX +1, INT 0, CHA 0, PER 0, LCK 0
        ...
        assert character.perception == 10  # 10 + 0
        assert character.luck == 10  # 10 + 0
    ...
    @patch('builtins.input', side_effect=["H", "Hero", "1", "1", "10", "12", "8", "10", "10", "10", "yes"])
    ...
    @patch('builtins.input', side_effect=["Hero", "1", "1", "0", "10", "12", "8", "10", "10", "10", "yes"])
    def test_create_character_stat_retry_on_invalid(self, mock_input):
        """Test: Retry prompt on invalid stat value (spec requirement)"""
        # Flow: name -> class -> method -> invalid str -> valid str -> dex -> int -> cha -> per -> luck -> confirm
        character = create_character()
        assert character is not None
    ...
    @patch('builtins.input', side_effect=["Hero", "1", "1", "10", "12", "8", "10", "10", "10", "no"])
...
class TestCreateCharacterWithClass:
    ...
    def test_create_character_with_warrior_class(self, mock_input):
        ...
        assert character.name == "Hero"
        assert character.character_class == CharacterClass.WARRIOR
        # Warrior bonuses: STR +3, DEX +1, INT 0, CHA 0, PER 0, LCK 0
        assert character.strength == 13  # 10 + 3
        assert character.dexterity == 13  # 12 + 1
        assert character.intelligence == 8  # 8 + 0
        assert character.charisma == 10  # 10 + 0
