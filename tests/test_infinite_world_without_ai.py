...
When a player moves in a direction where no location exists, a new location should be
generated on-demand at the target coordinates.

Movement is coordinate-based - moving in a direction calculates target coordinates
and either moves to an existing location or generates a new one.
"""
...
def world_with_coordinates():
    ...
    The starting location at (0,0) has Forest at (0,1) to the north.
    Moving in other directions will trigger fallback generation.
    """
    grid = WorldGrid()
...
def single_location_world():
    ...
    The location is at (0,0). Moving in any cardinal direction will
    trigger fallback generation since no locations exist at adjacent coordinates.
    """
    grid = WorldGrid()
...
class TestGenerateFallbackLocation:
    ...
    def test_generates_location_with_coordinates_for_navigation(self):
        """Spec: Fallback locations have coordinates for navigation back to source."""
        source = Location(
            name="Town Square",
            description="A town square.",
            coordinates=(0, 0)
        ...
        # New location at (0,1) can navigate south to source at (0,0)
        # Navigation is coordinate-based, not connection-based
        assert new_location.coordinates == (0, 1)
        assert source.coordinates == (0, 0)
        # Moving south from new_location reaches source
        target_y = new_location.coordinates[1] - 1  # south = -y
        assert target_y == source.coordinates[1]
...
class TestMoveToUnexploredDirection:
    ...
    def test_move_to_unexplored_direction_succeeds(
        ...
        """Spec: Moving in a valid direction with no location creates a new location."""
        ...
        # East has no location at (1,0), should create fallback location
    ...
    def test_move_to_unexplored_allows_return(
        self, basic_character, world_with_coordinates
    ):
        """Spec: Moving to unexplored direction allows return via coordinates."""
        world, starting_location = world_with_coordinates
        ...
        # Verify coordinate adjacency for return trip
        # New location at (-1,0), source at (0,0) - east returns to source
        assert new_location.coordinates[0] == source_location.coordinates[0] - 1
        assert new_location.coordinates[1] == source_location.coordinates[1]
...
class TestWorldNeverBecomesClosed:
    ...
    def test_fallback_location_has_coordinates(
        self, basic_character, single_location_world
    ):
        """Spec: Generated fallback locations have coordinates for further expansion."""
        ...
        # Should have coordinates for coordinate-based navigation
        assert new_location.coordinates is not None
        # Should be at (0,1) - north of origin
        assert new_location.coordinates == (0, 1)

    def test_multiple_expansions_maintain_coordinates(
        self, basic_character, single_location_world
    ):
        """Spec: World always has valid coordinates after multiple expansions."""
        world, starting_location = single_location_world
        ...
        # Perform multiple expansions in different directions
        game_state.move("north")  # (0,1)
        game_state.move("east")   # (1,1)
        game_state.move("south")  # (1,0)

        # After all moves, current location should have valid coordinates
        current = game_state.get_current_location()
        assert current.coordinates is not None
        assert current.coordinates == (1, 0)
    ...
    def test_single_location_start_allows_all_directions(
        ...
        # Should be able to move in all cardinal directions (generates fallback)
...
class TestInfiniteWorldIntegration:
    ...
    def test_exploration_and_return(
        ...
        """Spec: Can explore forward and return using coordinate-based navigation."""
        world, starting_location = single_location_world
        ...
        # Should have created new locations
    ...
    def test_exploration_expands_world(
        ...
        """Spec: Exploring expands the world with new locations."""
        world, starting_location = single_location_world
        ...
        # Move in multiple directions to expand the world
    ...
    def test_existing_locations_still_work(
        self, basic_character, world_with_coordinates
    ):
        """Spec: Existing locations work normally alongside fallback generation."""
        world, starting_location = world_with_coordinates
        ...
        # Move to existing location (north to Forest at (0,1))
        ...
        # Now move to unexplored direction (east to (1,0))
        success3, _ = game_state.move("east")
        assert success3 is True
        assert game_state.current_location != starting_location
        assert game_state.current_location != "Forest"
