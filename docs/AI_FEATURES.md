...
3. **AI World** (`ai_world.py`)
   ...
   - **Procedural layouts with ContentLayer**: Interior layouts are now generated using a two-stage pipeline:
     1. **Procedural generation** (`procedural_interiors.py`): Creates spatial layouts using category-specific algorithms
     2. **ContentLayer population** (`content_layer.py`): Transforms room templates into populated locations with names, descriptions, and content
   - **Category-specific generators**:
     - **BSPGenerator** (dungeons, temples, ruins, tombs, crypts, monasteries, shrines): Binary Space Partitioning for maze-like room structures
     - **CellularAutomataGenerator** (caves, mines): Organic cave-like chambers
     - **GridSettlementGenerator** (towns, villages, cities, settlements, outposts, camps): Grid-based street layouts
     - **TowerGenerator** (towers): Vertical multi-floor layouts with boss at top
   - **Deterministic generation**: An optional `seed` parameter enables reproducible interior generation - same seed produces identical layouts
